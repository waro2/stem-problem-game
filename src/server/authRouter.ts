...
export interface AuthDatabase {
  user: {
    upsert: (args: {
      ...
      select: { id: true; name: true; email: true; role: true; cohortId: true; cohort: { select: { name: true } }; cohortsManaged: { select: { id: true } }; consentGivenAt: true; analyticsConsent: true };
    }) => Promise<{ id: string; name: string | null; email: string; role: UserRole; cohortId: string | null; cohort: { name: string } | null; cohortsManaged: { id: string }[]; consentGivenAt: Date | null; analyticsConsent: boolean | null }>;
    update: (args: {
      where: { id: string };
